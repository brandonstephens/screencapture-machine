 ...
 *
 * usage : phantomjs responsive-screenshot.js {url} [output format] [doClipping] [outputBaseDir]
 *
 * examples >
...
/**
 * set if outputBaseDir ?
 * @type {string}
 */
var outputBaseDir = getOutputBaseDir();
	...
	if ( 'success' !== status ) {
		...
		function render(n) {
			if ( !!n ) {
				...
				output = outputBaseDir + folder + "/" + getFileName(viewports[key]);
